...
$envLines.Add((Format-EnvLine -Key "EBIO_SYNC_INTERVAL" -Value $syncInterval))
$envLines.Add((Format-EnvLine -Key "EBIO_COMPANY_SLUGS" -Value $companySlugs))
$envLines.Add((Format-EnvLine -Key "EBIO_NEW_EMPLOYEE_COMPANY_SLUG" -Value $newEmployeeCompanySlug))
$envLines.Add((Format-EnvLine -Key "EBIO_EMAIL_DOMAIN_XORORA" -Value $emailDomainXorora))
$envLines.Add((Format-EnvLine -Key "EBIO_EMAIL_DOMAIN_CREST_LED" -Value $emailDomainCrestLed))
$envLines.Add((Format-EnvLine -Key "EBIO_NAME_MATCH_THRESHOLD" -Value $nameMatchThreshold))

$envContent = ($envLines.ToArray() -join [Environment]::NewLine) + [Environment]::NewLine
